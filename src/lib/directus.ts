...
// === ПРОДУКТИ для Sitemap (Нова функція або адаптований getProducts) ===

export async function getProductsForSitemap() {
  // Нам потрібні:
  // 1. slug товару
  // 2. Дата оновлення (для lastModified)
  // 3. Усі пов'язані slug категорії (щоб побудувати URL)

  const params = new URLSearchParams({
    "filter[active][_eq]": "true",
    fields: "slug,date_updated,subcategories.category.slug", // <-- Фокусуємося на цих полях
    limit: "-1", // <-- Дуже важливо: щоб отримати всі товари
  });

  const url = `/items/products?${params.toString()}`;
  const data = await directusFetch<any[]>(url);

  // Обробка даних, щоб отримати масив пар (категорія, продукт)
  const productPaths = data.flatMap(product => {
    // В Directus товар може мати кілька підкатегорій, а підкатегорія - категорію
    const categorySlugs = product.subcategories
      .map((sub: { category: { slug: any; }; }) => sub.category.slug)
      .filter((v: any, i: any, a: string | any[]) => a.indexOf(v) === i); // Унікальні slug категорій

    // Створюємо окремий запис Sitemap для кожного шляху
    return categorySlugs.map((categorySlug: any) => ({
      categorySlug: categorySlug,
      productSlug: product.slug,
      lastModified: product.date_updated, // Використовуйте поле 'date_updated' Directus
    }));
  });
...
// === КАТЕГОРІЇ для Sitemap ===
export async function getCategoriesSlugs() {
  const data = await directusFetch<any[]>(
    `/items/categories?filter[active][_eq]=true&fields=slug`
  );
